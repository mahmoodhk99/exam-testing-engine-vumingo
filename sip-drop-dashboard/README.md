...
**A) Interactive (pick your own files) — `dashboard.html`**
Open **`dashboard.html`** (keep `xlsx.full.min.js` next to it). Choose your
**contact detail** `.xlsx` and **POM Agent Summary** `.xls`, click
**Analyze**, and the page reads and calculates everything in the browser — no
rebuild, no server. Use this to run the report on new files yourself.

**B) Pre-built snapshot — `index.html`**
Open **`index.html`** — a self-contained page with the current data already
embedded (generated by `build.py`).

Both pages look and calculate identically.

## How to use (the pre-built page)
